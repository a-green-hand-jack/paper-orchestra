...
已有成功产物会按当前检查与缓存规则复用。恢复不会清空累计预算，也不能通过 `resume` 更换模型、模板或材料。需要改变这些输入时，创建新的输出工作区，保留旧结果作为对照。累计 token 上限可以按下文的显式授权方式提高，其他预算不随之改变。
...
预算耗尽后，原结果会保留；普通 `resume` 不会重置或提高预算。明确同意增加 token 额度时，可以从原工作区继续：

```bash
paper-orchestra resume ./paper-run --headless --allow-lkm-spend --max-total-tokens 16000000
```

这是把全程有效上限提高到 1600 万，不是额外赠送 1600 万，也不会清零已用量。系统在预算账本中记录旧上限、新上限和当时用量；原始 scope 保持不变，有效上限以 `budget.limits` 为准。不指定此参数就不提高上限，同值重复请求不会重复增加，较低值会被拒绝。模型、时间、图像等其他预算不变，不要手工修改状态文件。
